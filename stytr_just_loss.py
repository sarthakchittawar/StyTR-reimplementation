...
class StyTrans(nn.Module):
    ...
    def forward(self, content: NestedTensor, style: NestedTensor, final: NestedTensor):
        ...
        final_features = self.intermediate_encoding(final.tensors)
            
        # content loss
        content_loss = 0
        for i in range(5):
            content_input = content_features[i]
            m, s = self.calc_mean_std(content_input)
            normalised_content_input = (content_input - m) / s
            content_target = final_features[i]
        ...
        style_loss = 0
        for i in range(5):
            style_loss += self.calc_style_loss(style_features[i], final_features[i])            
